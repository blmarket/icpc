...
int enter(string line) {
    int mincost = -1;
    int md = -1;
    for(int i=0;i<size(undos);i++) {
        if(undos[i].size() > line.size()) continue;
        int sz = size(undos[i]);
        if(line.substr(0, sz) != undos[i]) continue;

        int tmp = size(line) - sz;
        if(i+1 < size(undos)) tmp += 2;
