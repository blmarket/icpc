...
public:
	string solve(vector <string> cake, vector <string> cutter) 
	{
            ...
            for(int i=0;i<size(cake);i++)
            {
                for(int j=0;j<size(cake[i]);j++)
                {
                    if(cake[i][j] == '.')
                    {
                        int x = i;
                        int y = j - firstpos;
                        for(int a=0;a<size(cutter);a++)
                        {
            ...
            for(int i=0;i<size(cake);i++) for(int j=0;j<size(cake[i]);j++) if(cake[i][j] == '.') return "NO";
            return "YES";
	}
